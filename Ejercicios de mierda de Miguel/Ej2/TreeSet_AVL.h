...
protected:
   ...
   struct TreeNode {
      ...
      Link iz, dr;
      int altura;
      int tam_i; // numero de nodos en el hijo izquierdo + 1
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int tam_iz = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(tam_iz) {}
   };
...
public:
   ...
   T const& kesimo(int k) const {
      if (empty() || k < 1 || k > size()){
         throw std::invalid_argument("Element isn't in the tree");
      }
      return kesimoRecursivo(raiz, k);
   }
...
protected:
   ...
   T const& kesimoRecursivo(const Link& a, int k) const {
      ...
      if (k < a->tam_i){
         return kesimoRecursivo(a->iz, k);
      }
      //Buscamos por la derecha
      else{
         //Restamos tam_i por la forma en la que funciona tam_i
         return kesimoRecursivo(a->dr, k - a->tam_i);
      }
   }
