...
public:
	...
	T const& kesimo(int k) const {
		if (nelems < k)
			throw std::out_of_range ("No existe ese numero");
		return kesimo_int(k, raiz);
	}

protected:
	T const& kesimo_int(int k, Link a) const
	{
		if (a->tam_i == k) return a->elem;
		if (a->tam_i < k)
			return kesimo_int(k - a->tam_i, a->dr);
		else return kesimo_int(k, a->iz);
	}
